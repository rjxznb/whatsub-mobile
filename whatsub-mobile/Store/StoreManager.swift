...
final class StoreManager: ObservableObject {
    ...
    func loadProducts() async {
        do {
            let products = try await Product.products(for: [Self.buyoutProductID])
            buyoutProduct = products.first
        } catch {
    ...
    func purchaseBuyout() async -> Bool {
        guard let product = buyoutProduct else {
    ...
    private func refreshLocalEntitlements() async {
        var found = false
        for await result in Transaction.currentEntitlements {
            if case .verified(let t) = result, t.productID == Self.buyoutProductID {
                found = true
            }
        }
        hasLocalBuyout = found
    }
}
