...
**Cancellation (2026-07-20)** — auto-sync made "close the sheet" silently dangerous: the View span a detached `Task {}`, so dismissing only tore down the UI while the run finished and uploaded a cloud entry the user believed they'd cancelled, burning one of the 3 free video slots. Now `ImportViewModel` OWNS the task (`start` / `startRetryAnalysis` / `cancelWork`), `AnalysisEngine.analyze` calls `Task.checkCancellation()` at entry + between batches (cheapest bail point — the previous batch's tokens are already spent) + before phase 2, and `performAnalysis` re-checks immediately before `sync()` (that last check is what actually keeps a cancelled import out of the quota). `ImportView`'s ROOT `.onDisappear` fires `cancelWork()`; sheets presented ON TOP (diagnostics / VPN help) don't fire it, so they can't kill an in-flight run. **Gotcha worth remembering:** phase 2's `catch` deliberately swallows summary failures — it also swallowed `CancellationError`, letting a cancel land, return a result, and sync anyway. It now re-throws `CancellationError` explicitly. Caption disk cache is untouched by cancellation (it lands before analysis starts; harmless + makes a re-import instant).

### VPN routing UX (Chinese users, 2026-06-21; reworked 2026-07-20)

iOS can't bypass system VPN per-request (no public API — `URLSessionConfiguration.connectionProxyDictionary` only covers HTTP proxies, not packet tunnels), so the fix necessarily lives on the user's VPN side.

**The insight that reframed this (2026-07-20):** users believed they had to toggle VPN on/off constantly (YouTube needs it, our API breaks under it) — a top acquisition blocker. They don't. Both our hosts resolve to Chinese Aliyun IPs (`whatsub.eversay.cc` → 47.93.87.206; `cdn.eversay.cc` → Aliyun CDN nodes), and essentially every mainstream rule set ships `GEOIP,CN,DIRECT` — so a client in **规则/分流 mode** already routes whatSub direct and YouTube through the proxy, simultaneously, zero config. Only **全局 mode** breaks us. NOTE it is GEOIP that saves us, *not* any vendor domain rule: `eversay.cc` is our own domain, so rule sets listing `aliyun.com`/`alicdn.com` never match it — which is why the explicit rule must stay available for the setups GEOIP can't rescue (fake-ip DNS where `GEOIP,...,no-resolve` sees no real IP; slimmed rule sets with no `GEOIP,CN`).

- **`VPNRuleHelpSheet.swift`** — two tiers. Tier 1 (hero card): "把 VPN 切到规则模式就行", nothing to copy, plus where each client hides its mode switch + a labelled Shadowrocket screenshot. Tier 2 (collapsed, "切了还是不行？"): the `DOMAIN-SUFFIX,eversay.cc,DIRECT` snippets per client (one suffix rule covers BOTH `whatsub.*` and `cdn.*`), with a 3-screenshot Shadowrocket walkthrough (配置文件 ⓘ → 规则 + → DOMAIN-SUFFIX/DIRECT/eversay.cc → 保存). Screenshots live in `Assets.xcassets/VPNGuideShadowrocket` + `VPNRuleShadowrocket{1,2,3}`; both `screenshotView` and `stepStrip` gate on `UIImage(named:)` so a client without assets renders text-only instead of an empty frame. **When adding screenshots, crop first** — the original Shadowrocket home shot carried the user's own 机场 domain, remaining traffic and expiry date, which must never ship to all users.
- **Entry points (2026-07-20)** — was reachable only from error paths, i.e. invisible to the users who needed it most. Now: one-time onboarding on first launch **only when `VPNDetector.isVPNActive()`** (so non-VPN users never see it; deferred behind the AI-consent sheet, retried next launch if it didn't show); a permanent 我的 → 工具 row; the Library load-error state naming the likely cause when a tunnel is up; and a tappable 需VPN badge on Library rows (`.borderless` so it doesn't fire the row's NavigationLink).
- **`VPNDetector.swift`** — `CFNetworkCopySystemProxySettings()["__SCOPED__"]` keys matched against `utun`/`tun`/`tap`/`ppp`/`ipsec` prefixes. Public-API, App-Store-safe heuristic; only gates whether to SHOW guidance, so a false positive costs one dismissible sheet. Classification is a pure function (`containsVPNInterface`) with unit tests.
- **`RelayLoadingView.swift`** — 5s stall hint. Normal spinner + label first; after 5s without progress flips to a yellow shield icon + "可能 VPN 拦了 eversay.cc" copy + 「查看 VPN 设置方法」 button. No active probe of eversay.cc — the real network call IS the probe. Wired into Library 详情→收藏 tab (`EntryCollectionsList`) and Library 详情→角色扮演 tab (`RoleplayTabView`).
...
### Library list cache (2026-07-05)

The Library list had NO cache: cold start spun on the network, offline showed an error page, and every pull-to-refresh refetched everything (backend also re-signs a CDN URL per entry) even when nothing had changed. Now mirrors the corpus cache-first flow: `LibraryCache` (`Caches/library_cache.json`) holds entries + the server fingerprint; refresh hits `GET /api/library/version` (one number, `MAX(synced_at)+COUNT(*)`) and only refetches `/list` when it differs. Two deliberate differences from `CorpusCache`: `fetchedAt > 0` (not `entries.isEmpty`) marks "has cache", because an EMPTY library is a valid cached state for a new user; and the cache records `ownerEmail`, checked on every read, so logout→login as a different account can never render the previous user's list. Version probe is best-effort — any failure falls back to the full fetch, so an old backend still works. `delete()` clears the cache so a cold start can't resurrect a removed row. Cached `videoUrl`/`audioUrl` are 2h signed URLs and go stale, which is fine: playback re-fetches `/entry/:id` for fresh ones, and the list only reads `videoUrl`'s nil-ness for the 免VPN badge.

### Desktop-presence hint for the import queue (2026-07-20)

Pushing a URL to the desktop queue when the desktop app isn't running left the task queued silently forever — the only clue was a line of static copy. The backend now records `desktop_presence` (email → last_seen), bumped by the requests only the DESKTOP makes: its 30s poll `GET /import-queue?status=pending` (iOS lists WITHOUT a status filter), claims, and non-pending status writes (`status='pending'` is ALSO the iOS retry button, deliberately excluded). `POST` + `GET /import-queue` return `desktopSeenSecondsAgo` (nil = never seen / old backend). iOS treats >120s (4 missed polls) or nil as offline: the push-success screen shows a yellow "你的桌面端似乎不在线" callout naming the account to sign into, and `ImportQueueView` shows a top banner when items are waiting. Zero desktop-client changes.

**Import screen button order (2026-07-20)** — 手机解析 is now the primary action (top slot, solid accent, "推荐 · 无需电脑"); 推送到桌面端 dropped to an outline secondary. With the desktop button on top, most users picked the path that needs a running desktop. Trade-off to watch: phone-parsed entries are YouTube-embed (need VPN to watch) while desktop-parsed ones are OSS-hosted (免VPN) — if "导入了却看不了" complaints appear, that's this change surfacing.
...
- **You CANNOT auto-launch the containing app from a Share Extension on current iOS. Stop trying — this has now cost three attempts.** (settled 2026-07-20, device-verified). Every avenue is closed: `UIApplication.shared` is compile-time unavailable in extensions (`NS_EXTENSION_UNAVAILABLE_IOS`); `NSExtensionContext.open(_:)` is documented only for the *Today* extension point and typically calls back `false` doing nothing from a share sheet; and the classic responder-chain `perform("openURL:")` trick is dead because an extension runs in its own process with no `UIApplication` anywhere on the chain (Apple closed this around iOS 14). History: `2d6802d` added the responder-chain fallback claiming it enabled auto-launch; `042058d` removed it while fixing the YouTube freeze; `079fa18` restored it to bring auto-launch back — **device test showed it did nothing**, so it was reverted in `d8a43b8`. The original "the fallback makes it launch" claim was most likely never verified: the AppGroup + `scenePhase` safety-net makes the import screen appear the instant the user switches to whatSub manually, which is indistinguishable from a successful auto-launch. **Shipped behaviour (correct, leave it alone):** the extension writes the URL to the App Group and completes; the main app's `scenePhase == .active` handler reads `AppGroup.pendingImportURL()` and opens ImportView prefilled. The user taps whatSub themselves. This matches how most iOS apps behave — apps that appear to "jump" are either host-app integrations (YouTube/Instagram calling `UIApplication.open` from a *full app* via a partner URL scheme) or extensions that finish the job in place so no jump is needed. **Rejected alternative:** having the extension push straight to the desktop import queue (removing the need to jump) — it would take away the user's choice between 手机解析 and 推送到桌面端, which is the whole point of the import screen.
